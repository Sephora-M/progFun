  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    // def, quote, lambda and application
     ...
     case 'cond::xs =>{
         xs match {
           case Nil => () 
           case 'else::z0:: Nil => eval(z0,env)
           case w::ws => {
             w match {
               case 'else::z::Nil => eval(z,env) 
               case  test::exp:: Nil => if ( eval(test, env) != 0 )  eval(exp,env)
               						else eval('cond::ws,env) 
               case other => sys.error("Bad arguments for cond")
             }
           }
         }
       }     
    ...
    case 'def :: nameArgs :: body :: rest :: Nil =>{

    nameArgs match{
    	case Symbol(name) :: args :: Nil => {
    		val argsList = List(args)
    		val body1 = 'lambda :: argsList :: body :: Nil
    				if(env==globalEnv) globalEnv = env.extendRec(name,  env1 => eval( body1, env1))
    				eval(rest, env.extendRec(name,  env1 => eval( body1, env1)))

    	}
    	
    	case Symbol(name) :: args => {
    		//val argsList = List(args)
    		val body1 = 'lambda :: args :: body :: Nil
    				if(env==globalEnv) globalEnv = env.extendRec(name,  env1 => eval( body1, env1))
    				eval(rest, env.extendRec(name,  env1 => eval( body1, env1)))
